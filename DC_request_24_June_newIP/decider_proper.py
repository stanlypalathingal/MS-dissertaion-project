...
def abort_reason(file,reason):
	text_file = open("/home/pi/Documents/details/"+file+".log", "a") 
	text_file.write("\n"+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+","+ reason) # write the time
	text_file.close()
'''
...
while(True):
	...
	if(mess=="done"):
		...
		temp_data=pd.read_csv("/home/pi/Documents/details/test.csv",delimiter=",", 
		...
		if(temp_data.shape[0]>1):
			...
			if(len(spoof_detect(original_combination,temp_data))==0):
				...
				if(test_acc>99.90):
					...
					f = open("/home/pi/Documents/details/test.csv", "w")
					f.truncate()
					f.close()
				else:
					...
					benchmark(name_of_dc)
					abort_reason("accuracy_issues",test_acc)
				...
				abort_reason("error"," \n")
				# text_file = open("/home/pi/Documents/details/error.log", "a") 
				# text_file.write("\n"+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"\n") # write the time
				# text_file.close()
				spoof_detect(original_combination,temp_data).to_csv("/home/pi/Documents/details/error.log",mode="a",index=False) # save the error details with time
				end=time.time()
				print("Time taken for the analysis after receving the data ",end-start)
